...
let boundaryObject = document.querySelector("#wi-outside-boundary");

const MINOPTIONSIDX = 26;
const OFFSET = 3;
const PROFILEIDX = 0;
...
function showIndicator(f, hilightId, textId) {
  var theHilightId, theTextId, theTextChildren;
...
function resetFilter(theFilter, idx) {
  ...
  switch (theFilter) {
    case ACTIONS:
      ...
      for (let x = ENGAGE; x <= SPREADFOLD; x++) {
        // Engage - SpreadFold
        if (idx === x) {
...
function filter() {
  ...
  // scan and store all of the panel indicators which are set to on
  ...
  // two passes needed to filter
  removeIfNoMatch(btnCount, btnStatus, filteredArray);
  removeIfNoMatch(btnCount, btnStatus, filteredArray);
  ...
  for (let x = 0; x < filteredArray.length; x++) {
    for (let y = 0; y < filteredArray[x].length; y++) {
      if (btnStatus[y]) {
        if (filteredArray[x][y + OFFSET] === "0") {
          ...
          score += 10;
        } else if (filteredArray[x][y + OFFSET] === "2") {
          score += 0.1;
        }
      }
    }
    scoredPoints.push({
      score: score,
      id: parseInt(filteredArray[x][PROFILEIDX]),
      minOptions: parseInt(filteredArray[x][MINOPTIONSIDX + OFFSET])
    });
    score = 0;
  }
  ...
  for (let x = 0; x < tmpArray.length; x++) {
    let filteredObject = tmpArray[x];
    if (!isCompleteMatch(filteredObject, btnStatus)) {
      for (let y = 0; y < scoredPoints.length; y++) {
        let profileId = parseInt(tmpArray[x][PROFILEIDX]);
        let scoredPointsId = parseInt(scoredPoints[y].id);
        if (profileId === scoredPointsId) {
          removeObjectByIndex(scoredPoints, y);
          y = 0;
        }
      }
    }
  ...
  if (scoredPoints.length > 0) {
    ...
    finalChoice = filteredArrayItem;
    fltrInt = parseInt(filteredArrayItem[MINOPTIONSIDX + OFFSET]);
  }
  ...
  // now set filteredArray to the finalChoice which was found
  if (tmpFltr !== undefined) {
    filteredArray = tmpFltr;
  }

  hideAllPlots(); // reset plots
  let AIRCRAFTIDIDX = 1;
  let PLOTIDX = 32;
  let PITCHIDX = 30;
  let ROLLIDX = 31;
...
function findFilteredArrayIdx(filteredArray, id) {
  for (let x = 0; x < filteredArray.length; x++) {
    let fArrayId = parseInt(filteredArray[x][PROFILEIDX]);
    if (fArrayId === id) {
      return x;
    }
  }
...
function isCompleteMatch(filteredObject, btnStatus) {
  ...
  // time: 7 - 8 inclusive
  // position: 9 - 16 inclusive
  // action: 17 - 25 inclusive

  let result = 0;
  let filterOptions = 0;
  let fail = false;
  let filterMatches = 0;
  let minMatches = parseInt(filteredObject[MINOPTIONSIDX + OFFSET]);

  let optionalMatches = 0;

  let isWeightMatch = false;
...
function removeIfNoMatch(btnCount, btnStatus, filteredArray) {
  ...
  for (let optIdx = 0; optIdx < btnStatus.length; optIdx++) {
    for (let x = 0; x < filteredArray.length; x++) {
      ...
      // if the filter option for this option is 0 remove this chart
      if (
        fltr !== "1" &&
        fltr !== "2" &&
        btnStat &&
        btnCount < minOptionsChosen
      ) {
...
function initWeatherIndicator(f, Snap) {
  // this function initialises and places a randowm positioned weather indicator
  // Pseudo Code Flow
  // ================
  // get random x y position to place weather indicator
  //      get boundary of weather indicator
  //      check weather indicator within outer boundary
  // get boundary of current plot
  // check weather indicator is inside current plot boundary
  //      if true colour green
  //      if false colour red
